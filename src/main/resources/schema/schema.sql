    ...
    CREATE TABLE clothes_order (
        stock_id VARCHAR(6),
        order_id VARCHAR(6),
        FOREIGN KEY (stock_id) REFERENCES some_table(stock_id) ON UPDATE CASCADE ON DELETE CASCADE,
        FOREIGN KEY (order_id) REFERENCES some_other_table(order_id) ON UPDATE CASCADE ON DELETE CASCADE
    );
